...
useEffect(() => {
  ...
  axios.get('https://herblogg.vercel.app/api/posts')
    ...
    .catch(error => {
      ...
      setWork([]); // Fallback to an empty array to prevent crashing on network failure
    });
}, []);


  const filteredWork = work.filter((article) => article.category === "Work" );
      ...
      blog_data.filter((article) => article.category === "work"   
          );
